...
- **`custom`** (`src/adapters/custom_adapter.py`, `CustomAdapter`) —
  ...
  `ai_prompt` / `ai_on_failure` — see below — which `CustomAdapter`
...
The `adapters` process needs no restart for any instance change. Each
source's loop rebuilds its adapter from the DB row every iteration
(`_reload_adapter`), so a config or Policy edit takes effect on the next
poll; a `main()` supervisor re-scans `adapter_instances` every
`_SUPERVISOR_POLL_SECONDS` (15s) and starts a loop thread for any source
that's been added or enabled, while a removed/disabled source's own loop
exits by itself and the supervisor reaps it.
...
- **`ai_prompt`** — a per-source `str.format` override of the summarization
  ...
  → built-in default.
- **`ai_on_failure`** — what `actions.ai` does when there's no real
  provider summary (AI disabled, no valid provider, the call fails or returns
  nothing): `continue_with_contents` (default) copies the extracted contents
  verbatim and keeps flowing, but a provider call that *raises* still stalls
  the item; `use_title` stores the item's title instead (catching a provider
  exception too); `abort` stores nothing, records an `item.ai_aborted` audit
  event, and stops — the item is never chunked or transmitted. Seeded
  `use_title` for **senapred** (emergency alerts — something beats nothing),
  absent for **csn**. Honours the legacy boolean `ai_fallback_to_title`
  (`true` → `use_title`); existing senapred rows are migrated once
  (`_backfill_senapred_ai_on_failure`).
...
**long-running process**, not a one-shot script. A supervisor keeps one
loop thread per enabled `adapter_instances` row (re-scanned every 15s so
UI additions/removals take effect live), each polling on its assigned
Policy's fetch schedule. `Ctrl+C` (or `SIGTERM`) stops every loop and exits
cleanly.
...
- `ADAPTERS_DEFAULT_INTERVAL_SECONDS` (env/`/config`, default `10`) —
  ...
  `interval_seconds`.
- `LOG_LEVEL` (env/`/config → Logging`, default `INFO`) — shared by every
  service via `adapters.logsetup.configure_logging`; the runner re-reads it
  each supervisor tick, so a change applies without a restart.
