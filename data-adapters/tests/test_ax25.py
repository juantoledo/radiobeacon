...
def test_max_frame_content_bytes_basic_ascii_overhead():
    # "CD3DXZ-1>WXALRT:" is 17 ASCII bytes.
    available = max_frame_content_bytes(callsign="CD3DXZ-1", destination="WXALRT")

    assert available == AX25_HARD_LIMIT_BYTES - len("CD3DXZ-1>WXALRT:")


def test_max_frame_content_bytes_accounts_for_prefix_and_suffix():
    baseline = max_frame_content_bytes(callsign="CD3DXZ-1", destination="WXALRT")
    with_wrap = max_frame_content_bytes(
        callsign="CD3DXZ-1", destination="WXALRT", prefix=">> ", suffix=" [EXPERIMENTAL]"
    )
