...
def test_get_connection_seeds_csn_api_and_senapred_custom_instances(tmp_path):
    ...
    # csn keeps the default AI-failure behavior -- no key stored.
    assert "ai_on_failure" not in csn_config
    ...
    # seeded for emergency alerts: use_title on AI failure, and a
    # SENAPRED-tuned summarization prompt override (see
    # _SENAPRED_AI_PROMPT_DEFAULT).
    assert set(senapred_config.keys()) == {"code", "ai_on_failure", "ai_prompt"}
    assert senapred_config["ai_on_failure"] == "use_title"
...
def test_get_connection_backfills_senapred_ai_on_failure(tmp_path):
    """ai_on_failure is seeded 'use_title' for senapred, but the seed only runs
    on an empty table -- a pre-existing senapred row without it (possibly with
    the old boolean) is migrated once."""
    db_path = tmp_path / "radiobeacon.db"
    legacy_conn = sqlite3.connect(db_path)
    legacy_conn.execute(
    ...
    legacy_conn.execute(
        ...
        ("senapred", "custom",
         json.dumps({"code": "def fetch(config): return []", "ai_fallback_to_title": True})),
    )
    legacy_conn.commit()
    legacy_conn.close()
    ...
    assert sen["ai_on_failure"] == "use_title"
    assert "ai_fallback_to_title" not in sen


def test_backfill_leaves_explicit_senapred_ai_on_failure_untouched(tmp_path):
    db_path = tmp_path / "radiobeacon.db"
    legacy_conn = sqlite3.connect(db_path)
    legacy_conn.execute(
        "CREATE TABLE adapter_instances (source TEXT PRIMARY KEY, adapter_type TEXT "
    ...
    legacy_conn.execute(
        "INSERT INTO adapter_instances (source, adapter_type, config) VALUES (?, ?, ?)",
        ("senapred", "custom",
         json.dumps({"code": "def fetch(config): return []", "ai_on_failure": "abort"})),
    )
    legacy_conn.commit()
    legacy_conn.close()
    ...
    sen = json.loads(get_adapter_instance(conn, "senapred")["config"])
    assert sen["ai_on_failure"] == "abort"
