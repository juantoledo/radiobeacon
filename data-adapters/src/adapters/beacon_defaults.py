...
# Transmit-state monitor (beacon.tx_monitor): a read-only tail of the
# SvxLink log that feeds the dashboard's cosmetic "ON AIR" indicator by
# watching for `Turning the transmitter (ON|OFF)`. On by default but
# self-disabling -- if the log isn't present/readable (SvxLink on another
# host, a container, or not configured) the monitor stays dormant, writes
# no telemetry, and the UI shows nothing. Never affects transmission.
BEACON_TX_MONITOR_ENABLED_DEFAULT = "true"
BEACON_SVXLINK_LOG_PATH_DEFAULT = "/var/log/svxlink"
...
# How many days a rendered TTS/AFSK WAV clip is kept in BEACON_TTS_WAV_DIR
# before the transmit loop prunes it (beacon.__main__._prune_old_wavs,
# checked ~hourly). Every airing — and each frame chunk, and each repeat
# under an interval/cron Policy — writes a fresh timestamped file that is
# never otherwise removed; the dashboard/item pages let an operator play any
# of them back, so they need to survive a while. "0" disables pruning.
BEACON_TTS_RETENTION_DAYS_DEFAULT = "30"
