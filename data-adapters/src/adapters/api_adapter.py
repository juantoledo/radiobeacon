...
from .templating import dotted_placeholder_paths, resolve_dotted_path, safe_format
...
def _lookup_bare_field(item: dict[str, Any], bare_field: str) -> Any:
    """The raw value a bare {Name}/field_date_format placeholder reads.
    A plain (non-dotted) name is unchanged from before this function
    existed: `item.get(bare_field)`, soft None if absent. A dotted name
    (e.g. "estacion.fecha") looks up its first segment the same soft way —
    an entirely absent base key means "this item just doesn't have that
    optional data", not an error — but once the base key is present,
    further navigation goes through resolve_dotted_path, whose ValueError
    on a structural break is deliberately NOT caught here: it propagates
    out of FieldMapping.resolve, aborting this item (see that class's
    docstring)."""
    first, _, rest = bare_field.partition(".")
    if not rest:
        return item.get(bare_field)
    if first not in item:
        return None
    return resolve_dotted_path(item[first], rest)
...
class FieldMapping:
    ...
    `template` may also reach into nested JSON with dots —
    "{estacion.nombre}", or deeper, "{datosEstaciones.estacion.nombreEstacion}"
    — and a list encountered anywhere along such a path (including the
    bare-placeholder case above) is resolved as its first element
    automatically (see templating.resolve_dotted_path). Unlike every other
    kind of mapping problem (an unknown flat placeholder, a bad
    field_date_format), a dotted path that's missing a key or hits
    something that isn't a dict is NOT swallowed to "" — it raises, and
    ApiAdapter._map_item turns that into skipping just this one item
    (logged), the same graceful-batch behavior a missing `id` already gets.
    This is deliberately not a partial-render story: dotted paths are a
    brand-new capability with no existing config relying on them failing
    silently, so failing loudly is simpler and safer than inventing one."""
    ...
    def resolve(
        ...
        if self.field_date_format:
            ...
            if bare_field is not None:
                raw = _lookup_bare_field(item, bare_field)
                if raw is None:
                    return None
        ...
        # Pre-validate every dotted placeholder against `context` directly
        # (bypassing safe_format's swallow) so a broken one raises here and
        # aborts this item, instead of silently rendering "" the way an
        # unknown flat placeholder does — see this class's docstring and
        # templating.resolve_dotted_path. An unknown *base* name (`first`
        # not in context at all) is left to safe_format, same as any other
        # unknown flat placeholder.
        for first, rest in dotted_placeholder_paths(self.template):
            if first in context:
                resolve_dotted_path(context[first], rest)
...
def _map_item(
    ...
    for name in _MAPPED_FIELDS:
        try:
            fields[name] = cfg.mapping_for(name).resolve(item, name, extra_context)
        except (KeyError, IndexError, ValueError, TypeError) as exc:
            # A broken dotted path (see FieldMapping's docstring) is the
            # one mapping failure that isn't swallowed to "" — surface it
            # as a single clear reason so fetch()'s per-item catch skips
            # and logs this item exactly like a missing `id` already does.
            raise ValueError(f"field {name!r} mapping failed: {exc}") from exc
...
class ApiAdapter(DataSourceAdapter):
    ...
    `config` is parsed fresh on every fetch() call, and the adapters runner
    rebuilds the adapter from the DB row each loop iteration
    (adapters.__main__._reload_adapter), so editing an instance's config via
    the UI takes effect on the very next poll, with no process restart."""
    ...
    def _source_template_context(
        ...
        try:
            ...
            logger.warning(
                "could not resolve source display metadata for templating source=%s",
                self.source,
                exc_info=True,
            )
