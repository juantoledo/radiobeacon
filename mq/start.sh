...
# shell's foreground process group, which `until`/`timeout` nesting here
# reliably produces. A SIGTTIN-stopped process ignores SIGINT *and*
# SIGTERM (only SIGCONT/SIGKILL affect it), so without this redirect a
# single attempt can wedge forever, immune to both this loop's own
# `timeout` and the user's own Ctrl+C. `timeout`'s `-k 2` is defense in
# depth: if a future change ever reintroduces a hang some other way,
# force-kill 2s after the TERM it sends on its own.
until timeout -k 2 3 docker compose exec -T mosquitto mosquitto_pub -h 127.0.0.1 -t healthcheck -m ping -q 0 </dev/null >/dev/null 2>&1; do
  attempts=$((attempts + 1))
