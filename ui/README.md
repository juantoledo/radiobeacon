...
Set `UI_DASHBOARD_TX_STREAM_URL` (`/config` → UI) to an HTTP(S) audio
stream of SvxLink's transmit audio and, while the ON AIR monitor reports
the rig keyed (see the beacon README's SvxLink section), every open
dashboard plays it — with a mute toggle in the header and on the ON AIR
pill (per-browser, `localStorage`). Plays by default; the browser blocks
the first autoplay-with-sound until you've interacted with the page once.

`on-air.js` re-broadcasts its `/dashboard/tx-state` poll as a `rb:tx-state`
event; `tx-audio.js` reacts to it and plays `GET /dashboard/tx-stream`
(its own `<audio>`, independent of the feed player). That route is a
reverse-proxy — `ui.tx_stream.StreamHub` holds **one** upstream connection
and fans it out to all viewers, so a plain `ffmpeg … -listen 1` source is
enough; bind it to localhost. Capture/serve recipe (ALSA loopback or
PulseAudio monitor → ffmpeg/Icecast) in
[documentation/svxlink-txqueue-SETUP.md](../documentation/svxlink-txqueue-SETUP.md).
Blank URL = no audio, glow indicator only.
...
- **admin** — full access to everything (dashboard, items, config,
  ...
  `/users`).
- **user** — read-only viewer: the dashboard plus the **Items** list and
  item detail pages, including rendered-audio playback. Everything that
  changes state (item override / rearm / replay / retransmit, beacon
  on/off, the dashboard quick toggles, manual transmit) and `/config`,
  `/audit`, `/users`, `/dev` all answer 403.

Roles are a hierarchy (`admin` outranks `user`), so an admin satisfies
any route a `user` can reach.
...
| `UI_DASHBOARD_TX_STREAM_URL` | *(unset — no dashboard transmission audio; when set, an HTTP audio-stream URL the UI proxies + fans out at `/dashboard/tx-stream`)* |
