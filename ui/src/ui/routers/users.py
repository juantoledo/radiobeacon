...
def users_list_page(request: Request, conn: sqlite3.Connection = Depends(get_db)):
    return templates.TemplateResponse(request, "users_list.html", {"users": list_users(conn)})
...
def user_role_action(
    ...
    if user.role == "admin" and role == "user" and _enabled_admin_count(conn) <= 1:
        return templates.TemplateResponse(
            request,
            "users_edit.html",
            {"user": user, "error": "cannot demote the last remaining admin"},
            status_code=400,
        )
    set_user_role(conn, user_id, role, actor="ui.users")
    return RedirectResponse(
        url=f"/users?{urlencode({'msg': f'{user.username!r} is now {role}'})}", status_code=303
    )
...
def user_reset_password_action(
    ...
    return RedirectResponse(
        url=f"/users?{urlencode({'msg': f'password reset for {user.username!r}'})}", status_code=303
    )
...
def user_delete_action(request: Request, user_id: int, conn: sqlite3.Connection = Depends(get_db)):
    ...
    if user.role == "admin" and _enabled_admin_count(conn) <= 1:
        return templates.TemplateResponse(
            request,
            "users_list.html",
            {"users": list_users(conn), "error": "cannot delete the last remaining admin"},
            status_code=400,
        )
    delete_user(conn, user_id, actor="ui.users")
    return RedirectResponse(
        url=f"/users?{urlencode({'msg': f'{user.username!r} deleted'})}", status_code=303
    )
