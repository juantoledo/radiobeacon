...
def test_nav_hides_developers_link_for_non_admin_user(user_client, conn):
    set_setting(conn, "UI_DEV_TOOLS_ENABLED", "true")
...
def test_new_item_page_suggests_default_policy(client):
    import re

    response = client.get("/dev/items/new")

    match = re.search(r'id="policy"[^>]*value="([^"]*)"', response.text)
    assert match is not None
...
def test_edit_item_page_does_not_override_existing_policy_or_date(client, conn):
    conn.execute(
        "INSERT INTO items (source, item_id, fetched_at, policy, source_date_time, rawdata) "
...
def test_create_item_redirects_and_persists(client, conn):
    ...
    row = conn.execute(
        "SELECT extracted_title, policy FROM items WHERE source='csn' AND item_id='new-1'"
    ).fetchone()
...
def test_edit_item_page_renders_null_fields_as_empty_not_the_word_none(client, conn):
    # _insert_item only sets extracted_title/policy — every other
...
def test_update_item_action_redirects_to_detail_page(client, conn):
    ...
    response = client.post(
        "/dev/items/csn/1",
        data={"extracted_title": "Updated", "policy": "urgent"},
        follow_redirects=False,
    )
    ...
    row = conn.execute(
        "SELECT extracted_title, policy FROM items WHERE item_id='1'"
    ).fetchone()
...
def test_open_readonly_connection_blocks_writes_at_the_driver_level(tmp_path, monkeypatch):
    ...
    try:
        with pytest.raises(sqlite3.OperationalError):
            ro_conn.execute("UPDATE items SET policy = 'urgent'")
    finally:
        ro_conn.close()
