...
def test_logging_tab_is_the_log_level_group_form(client):
    response = client.get("/config/logging")

    assert response.status_code == 200
    assert "LOG_LEVEL" in response.text


def test_logging_group_save_persists_log_level(client, conn):
    ...
    assert get_setting("LOG_LEVEL", conn=conn) == "DEBUG"
