...
### Indicador "AL AIRE" del panel (opcional, cosmético)

El proceso `beacon` de radiobeacon puede mostrar un resplandor **AL AIRE** en vivo
en el panel del operador mientras SvxLink mantiene el transmisor activado. Lo hace
igual que este servicio rastrea el estado del canal — leyendo `/var/log/svxlink`
en busca de `Turning the transmitter (ON|OFF)` — y escribe el resultado en
`beacon_status` para que la interfaz lo renderice.

- Es de **solo lectura**: nunca toca SvxLink, el `COMMAND_PTY` ni la transmisión.
  Desactivarlo o que falle no cambia nada salvo el indicador.
- Necesita acceso de lectura al log. El proceso `beacon` normalmente no corre como
  el usuario `svxlink`, así que agrega su usuario al grupo `svxlink` (o `adm`), o
  apunta `BEACON_SVXLINK_LOG_PATH` a una copia legible por todos.
- Si el log no existe o no es legible — SvxLink en otro host, en un contenedor, o
  aún sin configurar — el monitor queda inactivo y el panel simplemente no muestra
  nada. La transmisión no se ve afectada.
- Se controla con `BEACON_TX_MONITOR_ENABLED` / `BEACON_SVXLINK_LOG_PATH` (`.env` o
  `/config` → Beacon — SvxLink).

### Audio de transmisión en vivo del panel (opcional)

Mientras el transmisor está activado, el panel también puede **reproducir un stream
en vivo del audio de transmisión de SvxLink** en el navegador, con un control de
silencio por visor. Define `UI_DASHBOARD_TX_STREAM_URL` (`/config` → UI) con una URL
HTTP(S) de audio; la interfaz hace de proxy inverso en `/dashboard/tx-stream` y
reparte una única conexión ascendente a todos los paneles abiertos, así que enlaza
el stream a `127.0.0.1`. En blanco = función desactivada (el resplandor sigue
funcionando).

**1. Captura lo que SvxLink emite.** El dispositivo de audio de TX de SvxLink es
solo de reproducción, así que enruta una copia a algo capturable:

- *PulseAudio / PipeWire*: captura directamente la fuente `.monitor` del sink de
  salida — `pactl list short sources | grep monitor`.
- *ALSA puro*: carga `snd-aloop`, apunta el `Tx` de SvxLink a `hw:Loopback,0,0`,
  puentea `hw:Loopback,1,0 → plughw:0,0` (la tarjeta USB real) con `alsaloop`, y
  captura `hw:Loopback,1,1`. (CONTEXT.md tiene el contexto de `plughw:0,0` / `dsnoop`.)

**2. Sírvelo como HTTP.** Lo más simple, una sola fuente, sin daemon extra — el
listener incorporado de `ffmpeg`:
...
`-listen 1` acepta un solo cliente — el hub de reparto de la interfaz *es* ese
cliente, y sirve a todos los visores desde él. Luego:

```
UI_DASHBOARD_TX_STREAM_URL=http://127.0.0.1:8123/svxlink.mp3
```

`ffmpeg -listen 1` **termina cuando ese cliente se desconecta** (el hub suelta la
conexión ascendente ~10 s después de que se va el último visor), así que ejecútalo
desde una unidad `systemd` con `Restart=always` (y `After=svxlink.service`); el
panel se reconecta en unos segundos cuando vuelve. Si corres la interfaz con varios
workers de uvicorn, o quieres un codificador siempre listo, pon **Icecast** delante.

**3.** El stream va unos segundos por detrás de la detección AL AIRE (sondeo del
monitor + búfer del stream), así que se pierden los primeros instantes de una
transmisión — es un monitor "¿funciona? ¿suena bien?", no una grabación.
