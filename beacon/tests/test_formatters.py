...
def test_format_frame_builds_tnc2_line():
    result = format_frame("hola mundo", callsign="N0CALL-1", destination="WXALRT")

    assert result.tnc2 == "N0CALL-1>WXALRT:hola mundo"
...
def test_format_frame_raises_when_over_hard_byte_limit():
    ...
    with pytest.raises(FrameTooLongError):
        format_frame(long_text, callsign="N0CALL-1", destination="WXALRT")


def test_format_frame_accepts_content_right_at_the_limit():
    # "N0CALL-1>WXALRT:" is 17 ASCII bytes -- pad to exactly 256 total.
    prefix_len = len("N0CALL-1>WXALRT:")
    body = "x" * (256 - prefix_len)

    result = format_frame(body, callsign="N0CALL-1", destination="WXALRT")
...
def test_format_frame_date_placeholder_pushing_over_limit_still_raises():
    ...
    with pytest.raises(FrameTooLongError):
        format_frame(body, callsign="N0CALL-1", destination="WXALRT", suffix=" {date}", date="22-08-2026 14:30")
...
def test_format_frame_counts_utf8_bytes_not_chars_for_accented_text():
    ...
    with pytest.raises(FrameTooLongError):
        format_frame(body, callsign="N0CALL-1", destination="WXALRT")


def test_format_voice_prefixes_callsign_via_template():
    result = format_voice(
        "un resumen corto", callsign="N0CALL-1", template="{callsign}. {text}", max_chars=500
    )

    assert result.text == "N0CALL-1. un resumen corto"
    assert result.truncated is False


def test_format_voice_no_op_when_under_max_chars():
    text = "texto breve"
    result = format_voice(text, callsign="N0CALL-1", template="{text}", max_chars=500)
...
def test_format_voice_renders_date_placeholder():
    result = format_voice(
        "un resumen corto", callsign="N0CALL-1", template="{callsign}. {text}. {date}",
        max_chars=500, date="22-08-2026 14:30",
    )

    assert result.text == "N0CALL-1. un resumen corto. 22-08-2026 14:30"


def test_format_voice_date_defaults_to_blank():
    result = format_voice(
        "un resumen corto", callsign="N0CALL-1", template="{callsign}. {text}. {date}", max_chars=500
    )
...
def test_format_frame_falls_back_to_empty_on_invalid_placeholder():
    ...
    result = format_frame(
        "hola mundo", callsign="N0CALL-1", destination="WXALRT", prefix="[{typeo}] ", type="alerta",
    )
...
def test_format_voice_wraps_text_with_prefix_and_suffix():
    result = format_voice(
        "un resumen corto", callsign="N0CALL-1", template="{text}", max_chars=500,
        prefix=">> ", suffix=" <<",
    )
...
def test_format_voice_renders_item_field_placeholder_in_template_and_prefix():
    result = format_voice(
        "un resumen corto", callsign="N0CALL-1", template="{callsign}. [{type}] {text}",
        max_chars=500, prefix="({extracted_title}) ",
        type="alerta", subtype="", extracted_title="Alerta importante", url="", source="", item_id="",
    )

    assert result.text == "N0CALL-1. [alerta] (Alerta importante) un resumen corto"
...
def test_format_voice_falls_back_to_empty_on_invalid_placeholder():
    ...
    call."""
    result = format_voice(
        "un resumen corto", callsign="N0CALL-1", template="{callsign}. {text}",
        max_chars=500, prefix="[{typeo}] ",
    )

    assert result.text == "N0CALL-1. un resumen corto"  # prefix fell back to ""
