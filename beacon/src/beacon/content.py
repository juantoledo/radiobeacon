...
from adapters.storage import get_adapter_instance, get_source_fields
...
def resolve_voice_replacements(conn: sqlite3.Connection, source: str) -> dict[str, str]:
    """The operator's saved pronunciation map for this source's adapter, or
    {} — an empty map (absent key, no adapter row, unparseable config, or a
    non-dict value) means the feature is simply off for this adapter; there
    is no fallback to any suggested set. Applied by beacon.__main__'s
    _transmit_voice_unit via adapters.voice_replacements.apply_voice_replacements
    to the resolved voice text, before formatting/truncation."""
    ...
    try:
        ...
        return {}
    replacements = cfg.get("voice_replacements") if isinstance(cfg, dict) else None
