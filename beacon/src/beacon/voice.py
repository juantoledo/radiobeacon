...
def _synthesize_speech_espeak(text: str, *, out_path: Path, voice: str) -> bool:
    ...
    try:
        ...
        logger.error("espeak-ng not found on PATH — cannot synthesize speech")
        return False
    except subprocess.TimeoutExpired:
        logger.error("espeak-ng timed out synthesizing speech")
        return False
    if result.returncode != 0:
        logger.error(
            "espeak-ng exited %d: %s", result.returncode, result.stderr.decode(errors="replace")
        )
        return False
    return True
...
def _synthesize_speech_piper(text: str, *, out_path: Path, model_path: str, binary: str) -> bool:
    ...
    if not model_path:
        logger.error("BEACON_TTS_PIPER_MODEL not configured — cannot synthesize speech via piper")
        return False
    try:
        result = subprocess.run(
        ...
        logger.error("piper not found on PATH — cannot synthesize speech")
        return False
    except subprocess.TimeoutExpired:
        logger.error("piper timed out synthesizing speech")
        return False
    if result.returncode != 0:
        logger.error(
            "piper exited %d: %s", result.returncode, result.stderr.decode(errors="replace")
        )
        return False
    return True
