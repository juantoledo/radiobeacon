...
def synthesize_frame_wav(
    ...
    *,
    ...
    """TNC2 line (e.g. "CD3DXZ-1>NFO:content") -> 16 kHz mono AFSK WAV at
    ...
    if not line:
        logger.error("frame_audio: empty TNC2 line, nothing to render")
        return False
    ...
    try:
        ...
        logger.error("frame_audio: %s not found on PATH — cannot render frame WAV", gen_packets_binary)
        return False
    except subprocess.TimeoutExpired:
        logger.error("frame_audio: %s timed out rendering frame WAV", gen_packets_binary)
        return False
    finally:
        try:
            tf_path.unlink()
    ...
    if not out_path.exists() or out_path.stat().st_size < 44:
        logger.error("frame_audio: %s produced no usable WAV at %s", gen_packets_binary, out_path)
        return False
...
def _prepend_silence(wav_path: Path, ms: int) -> None:
    ...
    try:
        ...
        logger.warning("frame_audio: could not prepend silence to %s, using as-is", wav_path, exc_info=True)
