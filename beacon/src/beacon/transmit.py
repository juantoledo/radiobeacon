...
class LoggingWavTransmitter:
    ...
    def transmit(self, *, wav_path: Path, label: str) -> bool:
        logger.info("transmit [logging]: would drop %s into the spool (%s)", wav_path, label)
        return True
...
class SpoolWavTransmitter:
    ...
    def transmit(self, *, wav_path: Path, label: str) -> bool:
        try:
            ...
            logger.error("transmit [spool]: failed to place %s in %s (%s)",
                         wav_path, self._incoming_dir, label, exc_info=True)
            return False
        logger.info("transmit [spool]: queued %s (%s)", final, label)
        return True
