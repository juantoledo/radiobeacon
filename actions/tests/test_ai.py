...
def test_ai_use_ai_defaults_to_true_when_unset(tmp_path, monkeypatch):
    """An adapter with no `use_ai` key (every adapter predating this field)
    is summarized exactly as before -- the global switch is still the only
    gate."""
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    monkeypatch.setenv("ACTIONS_AI_MAX_CHARS", "0")
    ...
    conn = get_connection(tmp_path / "radiobeacon.db")
    set_adapter_instance(conn, "alerts", "custom", {"code": "def fetch(config): return []"})
    _insert_item(conn, "alerts", "1", "Some contents.")

    outputs = AiAction().run(_dispatched_event("alerts", "1"), conn=conn)

    assert outputs[0]["summarized"] is True
    assert _stored_summary(conn, "alerts", "1") == "A summary."


def test_ai_use_ai_false_skips_without_calling_provider(tmp_path, monkeypatch):
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    called = []
    monkeypatch.setattr(
    ...
    conn = get_connection(tmp_path / "radiobeacon.db")
    set_adapter_instance(
        conn, "alerts", "custom", {"code": "def fetch(config): return []", "use_ai": False}
    )
    _insert_item(conn, "alerts", "1", "Some long contents.", extracted_title="A short title")
    ...
    assert outputs[0]["summarized"] is False
    assert outputs[0]["reason"] == "AI disabled for this adapter (Use AI unchecked)"
    assert called == []
    assert _stored_summary(conn, "alerts", "1") == "Some long contents."


def test_ai_use_ai_false_ignores_ai_on_failure_use_title(tmp_path, monkeypatch):
    """Use AI unchecked is a deliberate "this adapter's contents are fine
    without AI" choice, not a failure -- it always stores extracted_contents
    verbatim, regardless of whatever ai_on_failure happens to be set to."""
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    conn = get_connection(tmp_path / "radiobeacon.db")
    set_adapter_instance(
        ...
        "alerts",
        "custom",
        {"code": "def fetch(config): return []", "use_ai": False, "ai_on_failure": "use_title"},
    )
    _insert_item(conn, "alerts", "1", "Some long contents.", extracted_title="A short title")
...
def test_ai_use_ai_false_never_aborts_even_with_ai_on_failure_abort(tmp_path, monkeypatch):
    """Regression: an adapter configured with ai_on_failure=abort (set back
    when it still relied on AI, for a source whose raw contents were
    meaningless without it) must NOT have every item silently dropped the
    moment its own Use AI checkbox is unchecked -- that combination used to
    route through _handle_no_summary's "abort" mode, recording
    item.ai_aborted and returning [] for every item forever."""
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    conn = get_connection(tmp_path / "radiobeacon.db")
    set_adapter_instance(
        ...
        "alerts",
        "custom",
        {"code": "def fetch(config): return []", "use_ai": False, "ai_on_failure": "abort"},
    )
    _insert_item(conn, "alerts", "1", "Some long contents.", extracted_title="A short title")
...
def _aborted_rows(conn, source, item_id):
    return conn.execute(
        "SELECT details FROM audit_log WHERE event_type = 'item.ai_aborted' "
        "AND source = ? AND item_id = ?",
        (source, item_id),
    ).fetchall()


def test_ai_abort_on_provider_failure(tmp_path, monkeypatch):
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    monkeypatch.setenv("ACTIONS_AI_MAX_CHARS", "0")
    ...
    _insert_item(conn, "alerts", "1", "raw json blob", extracted_title="T")

    outputs = AiAction().run(_dispatched_event("alerts", "1"), conn=conn)

    assert outputs == []                                   # nothing published -> chunk never runs
    assert _stored_summary(conn, "alerts", "1") is None    # no summary stored
    rows = _aborted_rows(conn, "alerts", "1")
    assert len(rows) == 1 and "boom" in rows[0][0]


@pytest.mark.parametrize(
    "env, expected_reason_fragment",
    [
        ({"ACTIONS_AI_ENABLED": "false"}, "ACTIONS_AI_ENABLED"),
        ({"ACTIONS_AI_ENABLED": "true", "ACTIONS_AI_PROVIDER": "nonsense"}, "invalid"),
    ],
)
def test_ai_abort_on_skip_paths(tmp_path, monkeypatch, env, expected_reason_fragment):
    for k in ("ACTIONS_AI_ENABLED", "ACTIONS_AI_PROVIDER"):
    ...
    _insert_item(conn, "alerts", "1", "raw blob", extracted_title="T")

    outputs = AiAction().run(_dispatched_event("alerts", "1"), conn=conn)

    assert outputs == []
    assert _stored_summary(conn, "alerts", "1") is None
    rows = _aborted_rows(conn, "alerts", "1")
    assert len(rows) == 1 and expected_reason_fragment in rows[0][0]


def test_ai_abort_on_content_already_short(tmp_path, monkeypatch):
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    monkeypatch.setenv("ACTIONS_AI_MAX_CHARS", "500")  # content is shorter -> skip
    conn = get_connection(tmp_path / "radiobeacon.db")
    set_adapter_instance(
        conn, "alerts", "custom",
        {"code": "def fetch(config): return []", "ai_on_failure": "abort"},
    )
    _insert_item(conn, "alerts", "1", "short raw blob")

    outputs = AiAction().run(_dispatched_event("alerts", "1"), conn=conn)

    assert outputs == []
    assert _stored_summary(conn, "alerts", "1") is None
    assert len(_aborted_rows(conn, "alerts", "1")) == 1


def test_ai_abort_on_empty_provider_response(tmp_path, monkeypatch):
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    monkeypatch.setenv("ACTIONS_AI_MAX_CHARS", "0")
    monkeypatch.setattr(actions.ai, "_call_ollama", lambda p, m, h: "   ")
    conn = get_connection(tmp_path / "radiobeacon.db")
    set_adapter_instance(
        conn, "alerts", "custom",
        {"code": "def fetch(config): return []", "ai_on_failure": "abort"},
    )
    _insert_item(conn, "alerts", "1", "long enough raw contents to summarize", extracted_title="T")

    outputs = AiAction().run(_dispatched_event("alerts", "1"), conn=conn)

    assert outputs == []
    rows = _aborted_rows(conn, "alerts", "1")
    assert len(rows) == 1 and "empty response" in rows[0][0]


def test_ai_empty_response_continues_with_contents_by_default(tmp_path, monkeypatch):
    """The old behaviour aired a blank bulletin (summarized: True, summary='');
    now an empty response is treated as a skip under the default mode."""
    monkeypatch.setenv("ACTIONS_AI_ENABLED", "true")
    monkeypatch.setenv("ACTIONS_AI_PROVIDER", "ollama")
    monkeypatch.setenv("ACTIONS_AI_MAX_CHARS", "0")
    monkeypatch.setattr(actions.ai, "_call_ollama", lambda p, m, h: "")
    conn = get_connection(tmp_path / "radiobeacon.db")
    _insert_item(conn, "csn", "1", "Sismo de magnitud 4.2.")

    outputs = AiAction().run(_dispatched_event("csn", "1"), conn=conn)

    assert outputs[0]["summarized"] is False
    assert "empty response" in outputs[0]["reason"]
    assert _stored_summary(conn, "csn", "1") == "Sismo de magnitud 4.2."


def test_resolve_ai_on_failure(tmp_path):
    conn = get_connection(tmp_path / "radiobeacon.db")
    assert actions.ai._resolve_ai_on_failure(conn, "unknown") == "continue_with_contents"
    set_adapter_instance(conn, "a", "custom",
                         {"code": "x", "ai_fallback_to_title": True})
    assert actions.ai._resolve_ai_on_failure(conn, "a") == "use_title"
    set_adapter_instance(conn, "b", "custom", {"code": "x", "ai_on_failure": "abort"})
    assert actions.ai._resolve_ai_on_failure(conn, "b") == "abort"
    set_adapter_instance(conn, "c", "custom", {"code": "x", "ai_on_failure": "bogus"})
    assert actions.ai._resolve_ai_on_failure(conn, "c") == "continue_with_contents"
