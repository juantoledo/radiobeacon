...
def _effective_max_chars(
    ...
    if available < configured_max_chars:
        logger.warning(
            "ACTIONS_CHUNK_MAX_CHARS=%d would risk AX.25 frame overflow given the current "
            "beacon callsign/destination/prefix/suffix, clamping to %d bytes_available=%d",
...
class ChunkAction(Action):
    ...
    def run(self, event: dict[str, Any], *, conn: sqlite3.Connection) -> list[dict[str, Any]]:
        ...
        if not contents:
            logger.info(
                "no summary or extracted_contents, skipping source=%s item_id=%s",
                source,
                item_id,
            )
            return []
        ...
        logger.info(
            "split into %d chunk(s) source=%s item_id=%s max_chars=%d",
            len(pieces),
            source,
            item_id,
            max_chars,
        ...
        for i, piece in enumerate(pieces):
            logger.info(
                "chunk %d/%d source=%s item_id=%s text=%r",
                i + 1,
                len(pieces),
                source,
                item_id,
